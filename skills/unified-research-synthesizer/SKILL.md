...
If `customer_feedback` is not provided, list provisional psychographic hypotheses from `target_audience` and `competitor_data`, label them `INFERRED`, and note the missing voice-of-customer evidence.

### Step 4: Competitive Landscape Mapping
From `competitor_data` (or verified public sources when explicitly researched):
