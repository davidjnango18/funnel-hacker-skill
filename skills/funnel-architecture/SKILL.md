...
5. **KPI context** — Owned baseline when available; otherwise a labeled external reference or test hypothesis
...
- Any KPI references must be labeled, sourced, and reasonably comparable for the funnel type and traffic source
- Upsell products must logically complement the front-end offer — no random add-ons
- Revenue model calculations require supplied price, margin, and take-rate inputs; otherwise mark the model incomplete
