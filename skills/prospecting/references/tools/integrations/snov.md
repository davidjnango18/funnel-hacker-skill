...
```bash
node references/tools/clis/snov.js domain search --domain example.com --type all --limit 10
```

### Find a specific person's email

```bash
node references/tools/clis/snov.js email find --domain example.com --first-name John --last-name Doe
```

### Verify an email

```bash
node references/tools/clis/snov.js email verify --email john@example.com
```

### Find prospect by email

```bash
node references/tools/clis/snov.js prospect find --email john@example.com
```

### Add prospect to a list

```bash
node references/tools/clis/snov.js prospect add --email john@example.com --first-name John --last-name Doe --list-id 12345
```
...
```bash
# List all lists
node references/tools/clis/snov.js lists list

# Get prospects in a list
node references/tools/clis/snov.js lists prospects --id 12345 --page 1 --per-page 50
```

### Check domain technology stack

```bash
node references/tools/clis/snov.js technology check --domain example.com
```
...
```bash
# List campaigns
node references/tools/clis/snov.js drips list

# Get campaign details
node references/tools/clis/snov.js drips get --id 12345

# Add prospect to drip campaign
node references/tools/clis/snov.js drips add-prospect --id 12345 --email john@example.com
```
